...
{
   //HELLO!
   cout << "The window size of the ROB is " << windowSize << endl;
   ...
   //....Additional Stuff for dependent loads...............
   m_loads_dep_on_loads = 0;
   
   m_total_load_load_length = 0;
   m_loads = 0;   
   m_num_hitL1 = 0;
   m_num_hitL2 = 0;
   m_num_hitL3 = 0;
   m_num_hitDram = 0;
   m_num_hitanothercore = 0;
   m_num_LongLatency = 0;
   m_num_LLL1 = 0;
   m_num_LLL2 = 0;
   m_num_LLL3 = 0;
   m_num_LLDRAM = 0; 
   m_num_LLAnothercore = 0;
   m_total_chain_length = 0;
   m_producer_atomic = 0;
   m_consumer_atomic = 0;
   m_tot_address_chain_length = 0;
   m_prod_and_cons = 0;
   m_free_producers = 0;
   m_num_mem_barrs = 0;

   registerStatsMetric("rob_timer", core->getId(), "numLoadsDependentonLoads", &m_loads_dep_on_loads);   
   registerStatsMetric("rob_timer", core->getId(), "Load-to-LoadDistance", &m_total_load_load_length);   
   registerStatsMetric("rob_timer", core->getId(), "totalChainLength", &m_total_chain_length);
   registerStatsMetric("rob_timer", core->getId(), "numProducerAtomic", &m_producer_atomic);
   registerStatsMetric("rob_timer", core->getId(), "numConsumerAtomic", &m_consumer_atomic);
   registerStatsMetric("rob_timer", core->getId(), "totalAddressChainLength", &m_tot_address_chain_length);
   registerStatsMetric("rob_timer", core->getId(), "prodAndCons", &m_prod_and_cons);
   registerStatsMetric("rob_timer", core->getId(), "freeProducer", &m_free_producers);  

   registerStatsMetric("rob_timer", core->getId(), "numLoads", &m_loads);     
   registerStatsMetric("rob_timer", core->getId(), "numHitL1", &m_num_hitL1);
   registerStatsMetric("rob_timer", core->getId(), "numHitL2", &m_num_hitL2);
   registerStatsMetric("rob_timer", core->getId(), "numHitL3", &m_num_hitL3);
   registerStatsMetric("rob_timer", core->getId(), "numHitAnotherCore", &m_num_hitanothercore);
   registerStatsMetric("rob_timer", core->getId(), "numHitDram", &m_num_hitDram);

   registerStatsMetric("rob_timer", core->getId(), "numLongLatency", &m_num_LongLatency);  
   registerStatsMetric("rob_timer", core->getId(), "numLongLatencyL1", &m_num_LLL1);  
   registerStatsMetric("rob_timer", core->getId(), "numLongLatencyL2", &m_num_LLL2);  
   registerStatsMetric("rob_timer", core->getId(), "numLongLatencyL3", &m_num_LLL3);  
   registerStatsMetric("rob_timer", core->getId(), "numLongLatencyDRAM", &m_num_LLDRAM);  
   registerStatsMetric("rob_timer", core->getId(), "numLongLatencyAnotherCore", &m_num_LLAnothercore);
   
   registerStatsMetric("rob_timer", core->getId(), "numMemBarriers", &m_num_mem_barrs); 
   
   recordMemTrace = false;
   recordDependentLoads = false;
   recordDepLoadsShort = false;
   if(recordMemTrace) memTrace.open("memTrace.txt");
   if (recordDependentLoads) loadDepsTrace.open("loadDepsTrace.txt");
   if(recordDepLoadsShort) depLoadsShort.open("depLoadsShort.txt");
   //..........................................................
}
...
RobTimer::~RobTimer()
{
   ...
   if (recordMemTrace) memTrace.close();
   if (recordDependentLoads) loadDepsTrace.close();
   if (recordDepLoadsShort) depLoadsShort.close();
}
...
// returns the input load if a chain depdendency load is not found
// prodDistance is the sequence number distance between load and its producer load 
RobTimer::RobEntry* RobTimer::findRecursiveLoadDependency(RobEntry* entry, UInt64 &prodDistance, UInt64 &chainLength){
      assert(entry->uop->getMicroOp()->isLoad());
      RobEntry *inputEntry = entry;
      if(recordDependentLoads) printEntryInfo(entry, loadDepsTrace); 
      if(entry->uop->getDependenciesLength() == 0){
            if(recordDependentLoads) loadDepsTrace << "This entry has no dependencies"<< "\n"<<"\n";    
            prodDistance = 0;
            chainLength=0;
            return inputEntry;
      }
      ...
      chainLength = 0;
      RobEntry* prodEntry = entry;
      UInt64 sequenceNumber = 0;     
     
     // until we reach the head of ROB 
      while(sequenceNumber != rob[0].uop->getSequenceNumber()){ 
            chainLength++;  // increment the chain length when we inspect a new producer     
            sequenceNumber = prodEntry->uop->getDependency(0); // get previous producer sequence number        
            prodEntry = this->findEntryBySequenceNumber(sequenceNumber); // get new producer entry
            if(recordDependentLoads) printEntryInfo(prodEntry, loadDepsTrace); 

            // if new producer entry is load BINGO!
            // note that we count the load as a producer load, whether it is done or not because that is the program characteristic 
            // we do this dependency check before dependencies are removed from an instruction in the ROB in the simulate() function 
            if ((prodEntry->uop->getMicroOp()->isLoad()) /*&& (prodEntry->done == SubsecondTime::MaxTime())*/){                  
                  if(recordDependentLoads) loadDepsTrace << "Found a chain depdendency load"<<"\n"<<"\n";                  
                  if(recordDepLoadsShort){
                        if(m_core->getId() == 0){
                              depLoadsShort<< "Producer:    "; printEntryInfo(prodEntry, depLoadsShort);                              
                              depLoadsShort<< "Consumer:    "; printEntryInfo(entry, depLoadsShort); 
                        }                       
                  }
                  //Statistics on producers and consumers that are atomic 
                  if(entry->uop->getMicroOp()->isMemBarrier()) m_consumer_atomic++;
                  if(prodEntry->uop->getMicroOp()->isMemBarrier()) m_producer_atomic++;                 

                  prodDistance = entry->uop->getSequenceNumber() - prodEntry->uop->getSequenceNumber();
                  chainLength = chainLength + 1; // include the consumer load in the chain                         
                  return prodEntry;

                  // if new producer not a load and it has no more dependencies there is no load endpoint
            }else if (!(prodEntry->uop->getMicroOp()->isLoad()) && (prodEntry->uop->getDependenciesLength() == 0)){                  
                  if(recordDependentLoads) loadDepsTrace << "This load has no chain depdency ending in a load"<< "\n" << "\n";
                  prodDistance = 0;
                  chainLength = 0;
                  return inputEntry;                  
            }
...
UInt64 RobTimer::findLoadAddressGenerationLength(RobEntry* entry, bool& isConsumerOfLoad){
      assert(entry->uop->getMicroOp()->isLoad());
      isConsumerOfLoad = false;
      if(entry->uop->getDependenciesLength() == 0){
            ++m_free_producers;            
            return 0;
      } 
      ...
      while(sequenceNumber != rob[0].uop->getSequenceNumber()){
           ...
           if(prodEntry->uop->getMicroOp()->isLoad()) isConsumerOfLoad = true;

           chain_size++; // new producer increments the chain length 
           // if the new producer has no dependencies, quit. We have founf the entire chain 
           if(prodEntry->uop->getDependenciesLength() == 0) return (chain_size + 1); // include the input load          
      }

      return (chain_size+1); // hit the top of ROB
}

void RobTimer::printEntryInfo(RobEntry* e, std::ofstream& file){
      file<<std::right<<std::setw(10)<<e->uop->getSequenceNumber()<<"  ";
      if (e->uop->getMicroOp()->isLoad())
         file<<"LOAD      ";
      else if (e->uop->getMicroOp()->isStore())
         file<<"STORE     ";
      else
         file<<"EXEC ("<<std::right<<std::setw(2)<<e->uop->getExecLatency()<<") ";
      if (e->uop->getMicroOp()->getInstruction())
      {
         file<<std::hex<<e->uop->getMicroOp()->getInstruction()->getAddress()<<std::dec<<": "
                  <<e->uop->getMicroOp()->getInstruction()->getDisassembly();
         if (e->uop->getMicroOp()->isLoad() || e->uop->getMicroOp()->isStore())
            file<<"  {0x"<<std::hex<<e->uop->getAddress().address<<std::dec<<"}";
      }
      else
         file<<"(dynamic)";

      file << "      Execution Latency = " << e->uop->getExecLatency();

      if ((e->uop->getMicroOp()->isLoad()) || (e->uop->getMicroOp()->isStore())){
            file << "      HitWhere:" << HitWhereString(e->uop->getDCacheHitWhere())<< "\n"; 
      }                               

      else file << std::endl;            
   }

void RobTimer::printEntryInfo(RobEntry* e){
      ...
      std::cout << "      Execution Latency = " << e->uop->getExecLatency();

      if ((e->uop->getMicroOp()->isLoad()) || (e->uop->getMicroOp()->isStore())){
            std::cout << "      HitWhere:" << HitWhereString(e->uop->getDCacheHitWhere())<< "\n"; 
      }  
...
boost::tuple<uint64_t,SubsecondTime> RobTimer::simulate(const std::vector<DynamicMicroOp*>& insts)
{
   ...
   for (std::vector<DynamicMicroOp*>::const_iterator it = insts.begin(); it != insts.end(); it++ )
   {
      ...
      //...................Additional things for dependent loads Statistics...................
      if(entry->uop->getMicroOp()->isLoad()){
            UInt64 prodDistance =0;
            UInt64 chainLength = 0;
            ++m_loads;
            RobEntry *prodEntry = findRecursiveLoadDependency(entry, prodDistance, chainLength);
            m_total_load_load_length+=prodDistance; // even if there is no depdency, the value will be 0
            m_total_chain_length+=chainLength; // even if there is no dependency, value wil be 0
            
            // if we found and load-to load dependency chain
            if(prodEntry != entry){
                  ++m_loads_dep_on_loads;
                  bool isConsumer = false;
                  UInt64 address_chain_length = findLoadAddressGenerationLength(prodEntry, isConsumer);            
                  m_tot_address_chain_length += address_chain_length; 
                  if(isConsumer)  ++m_prod_and_cons;
            }
      }
      if (entry->uop->getMicroOp()->isMemBarrier()) {++m_num_mem_barrs;}
      //....................................................................................
      ...
      uint64_t deps_to_remove[8], num_dtr = 0;
       // .....................Additional things to remove producer loads......................
      // if there is recursive Load dependency, remove the dependency on the producer load 
      /*if (entry->uop->getMicroOp()->isLoad()){
            UInt64 prodDistance =0;
            RobEntry *prodEntry = findRecursiveLoadDependency(entry, prodDistance);
            
            if(prodEntry != entry){                     
                  // if there is a recursive producer load, remove the dependency(0) from this load
                  // this emulates removing dependency on producer load                     
                  UInt64 sequenceNumber = entry->uop->getDependency(0);            
                  entry->uop->removeDependency(sequenceNumber);                  
            }           
      }*/
      //......................................................................................
...
void RobTimer::issueInstruction(uint64_t idx, SubsecondTime &next_event)
{
      ...
      && uop.getDCacheHitWhere() == HitWhere::UNKNOWN)
   {
    ...
    // ............Get trace and statistics..........................
         if(uop.getMicroOp()->isLoad()){               
               if(uop.getDCacheHitWhere() == HitWhere::L1_OWN) ++m_num_hitL1;
               else if(uop.getDCacheHitWhere() == HitWhere::L2_OWN) ++m_num_hitL2;
               else if(uop.getDCacheHitWhere() == HitWhere::L3_OWN) ++m_num_hitL3;
               else if(uop.getDCacheHitWhere() == HitWhere::L3_SIBLING) ++m_num_hitanothercore;
               else if(uop.getDCacheHitWhere() == HitWhere::DRAM_LOCAL) ++m_num_hitDram; 
                           
               if(uop.isLongLatencyLoad()){
                     ++m_num_LongLatency;                     
                     if(uop.getDCacheHitWhere() == HitWhere::L1_OWN) ++m_num_LLL1;
                     else if(uop.getDCacheHitWhere() == HitWhere::L2_OWN) ++m_num_LLL2;
                     else if(uop.getDCacheHitWhere() == HitWhere::L3_OWN) ++m_num_LLL3;                     
                     else if(uop.getDCacheHitWhere() == HitWhere::L3_SIBLING)++m_num_LLAnothercore;
                     else if(uop.getDCacheHitWhere() == HitWhere::DRAM_LOCAL) ++m_num_LLDRAM;
               }                       
         }
         ...
         if((uop.getMicroOp()->isLoad()) || (uop.getMicroOp()->isStore())){
               if(recordMemTrace){
                     if(m_core->getId() == 0){
                           memTrace << now << "     "; printEntryInfo(entry, memTrace);
                     }                    
               }
         }
   //...............................................................................
   ...
   #ifdef DEBUG_PERCYCLE
      std::cout<<"ISSUE    "<<entry->uop->getMicroOp()->toShortString()<<"     " << entry->uop->getSequenceNumber() << "   latency="<<uop.getExecLatency()<<std::endl;
   #endif
