...
#include <iostream>
//..........................................CHANGE!!!.............................................................................
// I have added 2 extra parameters to all the sendMsg functions
//................................................................................................................................
#if 0
   extern Lock iolock;
   ...
   forward_failed(0)
{
   ...
   registerStatsMetric("directory", core_id, "prefetch-dropped-other-state", &m_prefetch_dropped_other_state); // CHANGE!!!
   registerStatsMetric("directory", core_id, "prefetch-dropped-req-list", &m_prefetch_dropped_req_list); // CHANGE!!!
...
DramDirectoryCntlr::handleMsgFromL2Cache(core_id_t sender, ShmemMsg* shmem_msg)
{
   ...
   switch (shmem_msg_type)
   {
      ...
      case ShmemMsg::WB_REP:
         ...
         break;
      //...............................CHANGE!!!..........................
      case ShmemMsg::PREFETCH_REQ:
      {     
            //cout << "[handleMsgFromL2CACHE] sending DRAM_PREFETCH_REQ" << endl;            
            core_id_t dram_node = m_dram_controller_home_lookup->getHome(address);
            getMemoryManager()->sendMsg(PrL1PrL2DramDirectoryMSI::ShmemMsg::DRAM_PREFETCH_REQ,
                      ...
                      shmem_msg->getWhere(),
                      shmem_msg->getPerf(),
                      ShmemPerfModel::_SIM_THREAD, shmem_msg->getIntraTileRequester(), shmem_msg->isPrefetch(), shmem_msg->getFakeAddress());            
            break;
      }        
      //...................................................................
...
DramDirectoryCntlr::handleMsgFromDRAM(core_id_t sender, ShmemMsg* shmem_msg)
{
   ...
   {  
      //..................................CHANGE!!!...............................................
      // I added the case ShmemMsg::DRAM_PREFETCH_REP and Tag Check.  This calls processDRAMPrefetchReply   
      //...........................................................................................
      case ShmemMsg::DRAM_READ_REP:      
         processDRAMReply(sender, shmem_msg);
         break;   
      
      case ShmemMsg::DRAM_PREFETCH_REP:
         processDRAMPrefetchReply(sender, shmem_msg);
         break;  
      case ShmemMsg::TAG_CHECK:
         processTagCheckFromDRAM(sender, shmem_msg);
         break;
      default:
         LOG_PRINT_ERROR("Unrecognized Shmem Msg Type: %u", shmem_msg_type);
         break;
   ...
   switch (curr_dstate)
   {
      ...
      case DirectoryState::MODIFIED:
         getMemoryManager()->sendMsg(ShmemMsg::FLUSH_REQ,
               ...
               HitWhere::UNKNOWN,
               NULL,
               ShmemPerfModel::_SIM_THREAD, shmem_req->getShmemMsg()->getIntraTileRequester(), shmem_req->getShmemMsg()->isPrefetch());
         break;
...
DramDirectoryCntlr::processExReqFromL2Cache(ShmemReq* shmem_req, Byte* cached_data_buf)
{
   ...
   switch (curr_dstate)
   {
      ...
      {
         //.....................CHANGE!!!..............................................
         //security for late prefetch just like in processShReqFromL2Cache
         LOG_PRINT("processExReqFromL2Cache(), address(0x%x)", address);
         if((curr_dstate==DirectoryState::EXCLUSIVE) && (requester == directory_entry->getOwner())){
               assert(cached_data_buf == NULL); 
               // treat as uncached 
               cout << "[processExReqFromL2Cache] Late prefetch for address " << address << endl;
         ...
         MYLOG("Send FLUSH_REQ>%d for %lx", directory_entry->getOwner(), address )
               ...
               shmem_req->getShmemMsg()->getIntraTileRequester(), shmem_req->getShmemMsg()->isPrefetch());
               break;
         }       
      }
...
DramDirectoryCntlr::processShReqFromL2Cache(ShmemReq* shmem_req, Byte* cached_data_buf)
{
   ...
   switch (curr_dstate)
   {
      ...
      {  
         //.................................CHANGE!!!................................................
         // If requester==directoryEntry->getOwner(), it indicates we took in a late prefetch and set
         // the directory state to EX whereas it should have been UNCACHED. 
         LOG_PRINT("processShReqFromL2Cache(), address(0x%x)", address);
         if(requester == directory_entry->getOwner()){
               // treat as uncached 
               cout << "[processShReqFromL2Cache] Late prefetch for address " << address << endl;
               retrieveDataAndSendToL2Cache(ShmemMsg::EX_REP, requester, address, cached_data_buf, shmem_req->getShmemMsg());               
               break;
         }         
      ...
      core_id_t receiver, IntPtr address, Byte* cached_data_buf, ShmemMsg *orig_shmem_msg)
{
   ...
   if (cached_data_buf != NULL)
   {
      ...
      getMemoryManager()->sendMsg(reply_msg_type,
            ...
            ShmemPerfModel::_SIM_THREAD, orig_shmem_msg->getIntraTileRequester(), orig_shmem_msg->isPrefetch());
   ...
   else
   {
          ...
          && directory_entry->getForwarder() != INVALID_CORE_ID)
      {
         ...
         getMemoryManager()->sendMsg(ShmemMsg::WB_REQ,
            ...
            NULL, 0,
            HitWhere::UNKNOWN, shmem_req->getShmemMsg()->getPerf(), ShmemPerfModel::_SIM_THREAD, 
            orig_shmem_msg->getIntraTileRequester(), orig_shmem_msg->isPrefetch());
         return;
      }
...
DramDirectoryCntlr::processDRAMReply(core_id_t sender, ShmemMsg* shmem_msg)
{
   ...
   getMemoryManager()->sendMsg(reply_msg_type,
         ...
         ShmemPerfModel::_SIM_THREAD, shmem_msg->getIntraTileRequester(), shmem_msg->isPrefetch());
...
//.......................................CHANGE!!!.............................................
void
DramDirectoryCntlr::processDRAMPrefetchReply(core_id_t sender, ShmemMsg* shmem_msg){
      IntPtr address = shmem_msg->getAddress(); //  This should be the prefetch address
      IntPtr fake_address = shmem_msg->getFakeAddress(); 
      core_id_t requester = shmem_msg->getRequester();
      HitWhere::where_t hit_where = shmem_msg->getWhere();    
      
      if(hit_where == HitWhere::MISS){// do nothing

      }
      else if (m_dram_directory_req_queue_list->size(address) >= 1){
            // change nothing
            hit_where = HitWhere::MISS;    
            ++m_prefetch_dropped_req_list;       
      }      
      else{
            // first get the directory entry of the address 
            DirectoryEntry* directory_entry = m_dram_directory_cache->getDirectoryEntry(address);
            // directory entry may not be there. We have to evict and create a new one                
            if (directory_entry == NULL){
                  //cout <<"No directory entry present " << endl;

                  // But first we need to create shmem_req out of shmem_msg and set its data buf to be NULL 
                  // otherwise there is assertion failure when forming shmem_req 
                  ShmemMsg* fake_msg = new ShmemMsg(shmem_msg);
                  fake_msg->setDataBuf(NULL); 

                  SubsecondTime msg_time = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD);
                  // make a local copy of the message 
                  ShmemReq* shmem_req = new ShmemReq(fake_msg, msg_time);
                  directory_entry = processDirectoryEntryAllocationReq(shmem_req);
            }
            assert(directory_entry);
            ...
            if(curr_dstate == DirectoryState::UNCACHED){                  
                  ...
                  directory_entry->setOwner(requester);
                  directory_block_info->setDState(DirectoryState::EXCLUSIVE);
                  hit_where = shmem_msg->getWhere();     
                  assert(hit_where == HitWhere::DRAM);
                  hit_where = (sender == requester) ? HitWhere::DRAM_LOCAL : HitWhere::DRAM_REMOTE;  
                  //cout << "Sent prefetch " << address << " to core  " << shmem_msg->getIntraTileRequester() << " at time " << getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD) << endl;
            }else{
                  hit_where = HitWhere::MISS; ++m_prefetch_dropped_other_state;                  
            }
                  
      }
  
     getMemoryManager()->sendMsg(ShmemMsg::EX_PREFETCH_REP,
                                  MemComponent::TAG_DIR, MemComponent::L2_CACHE,
                                  requester,
                                  requester,
                                  address,
                                  shmem_msg->getDataBuf(), shmem_msg->getDataLength(),
                                  hit_where,
                                  shmem_msg->getPerf(),
                                  ShmemPerfModel::_SIM_THREAD, shmem_msg->getIntraTileRequester(), shmem_msg->isPrefetch(), fake_address);    
}

void
DramDirectoryCntlr::processTagCheckFromDRAM(core_id_t sender, ShmemMsg* shmem_msg){
      IntPtr prefetch_address = shmem_msg->getAddress();
      DirectoryEntry* directory_entry = m_dram_directory_cache->getDirectoryEntry(prefetch_address, true); // we include the dram directory tag check time 

       if(directory_entry != NULL){
             DirectoryBlockInfo* directory_block_info = directory_entry->getDirectoryBlockInfo();
             DirectoryState::dstate_t curr_dstate = directory_block_info->getDState();
             if(curr_dstate == DirectoryState::UNCACHED){
                   getMemoryManager()->sendMsg(ShmemMsg::TAG_CHECK_REP,
                                  MemComponent::TAG_DIR, MemComponent::DRAM,
                   ...
                   getMemoryManager()->sendMsg(ShmemMsg::EX_PREFETCH_REP,
                                  ...
                                  shmem_msg->getPerf(),
                                  ShmemPerfModel::_SIM_THREAD, shmem_msg->getIntraTileRequester(), shmem_msg->isPrefetch(), shmem_msg->getFakeAddress());   
                  LOG_PRINT("Property data already exists on chip for prefetch address(0x%x)", prefetch_address);
