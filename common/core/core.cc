      ...
      SubsecondTime now)
{
   ...
   LOG_ASSERT_ERROR(final_time >= initial_time,
         ...
         itostr(initial_time).c_str());
   //HELLO!
   /*if(hit_where != HitWhere::L1I){
         printf("ADDR(0x%x)    %s      HitWhere: %s    data_size: %u\n", address, (mem_op_type==READ)?"READ":"WRITE", HitWhereString(hit_where), data_size);
  }*/  
    
   LOG_PRINT("HitWhere: %s", HitWhereString(hit_where));
...
Core::accessMemory(lock_signal_t lock_signal, mem_op_t mem_op_type, IntPtr d_addr, char* data_buffer, UInt32 data_size, MemModeled modeled, IntPtr eip, SubsecondTime now, bool is_fault_mask)
{
   ...
   if (modeled == MEM_MODELED_NONE)
      return makeMemoryResult(HitWhere::UNKNOWN, SubsecondTime::Zero());
         ...
         MemoryResult res=  initiateMemoryAccess(MemComponent::L1_DCACHE, lock_signal, mem_op_type, d_addr, (Byte*) data_buffer, data_size, modeled, eip, now);
         return res;
   }      
}
